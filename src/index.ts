...
class stashql {
  ...
  async queryHandler(req: typeof New_Request, res: typeof New_Response, next: typeof NextFunction) {      
    // console.log("cwd: ", process.cwd()); // -> "/Users/Simon/dirname-example/StashQL package
    if (!fs.existsSync(path.join(process.cwd(), "logs"))){
      fs.mkdirSync(path.join(process.cwd(), "logs"), (err: any) => {
        if (err) {
  ...
  async clearCacheHandler(req: typeof New_Request, res: typeof New_Response, next: typeof NextFunction) {
    try {
